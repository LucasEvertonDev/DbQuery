...
namespace SIGN.Query.Test
{
    ...
    public class DominioRepository
    {
        ...
        public void Select()
        {
            var query = _dominioRepository.Select().Top(1).Where(a => a.Codigo > 1 && a.Descricao.LIKE("TESTE_LIKE")).OrderBy(a => a.Descricao).GetQuery();
            Assert.AreEqual(query, "SELECT DISTINCT TOP(1) * FROM SignCi..CiDominio WHERE (CiDominio.Codigo > 1 AND CiDominio.Descricao LIKE '%TESTE_LIKE%') ORDER BY CiDominio.Descricao ASC");
        }
        ...
        public void Join()
        {
            var query = _dominioRepository.Select().Top(1)
                .Join<Dominio, ItemDominio>((Dominio, ItemDominio) => (Dominio.Codigo == ItemDominio.Codigo_Dominio))
                .Where<Dominio, ItemDominio>((Dominio, ItemDominio) => Dominio.Codigo > 1 && Dominio.Descricao.LIKE(dominio.Descricao) 
                    && (Dominio.Nome == dominio.Nome && Dominio.Descricao != null))
                .OrderBy<Dominio>((Dominio => Dominio.Codigo))
                .OrderBy<ItemDominio>((ItemDominio) => ItemDominio.Nome)
                .GetQuery();

            Assert.AreEqual(query, "SELECT DISTINCT TOP(1) * FROM SignCi..CiDominio INNER JOIN SignCi..CiItemDominio ON CiDominio.Codigo = CiItemDominio.Codigo_Dominio WHERE ((CiDominio.Codigo > 1 AND CiDominio.Descricao LIKE '%TESTE_LIKE%') AND (CiDominio.Nome = 'Teste Nome' AND CiDominio.Descricao IS NOT NULL)) ORDER BY CiDominio.Codigo ASC, CiItemDominio.Nome ASC");
        }
        ...
        {
            var query = _dominioRepository.Select().Top(1)
                .LeftJoin<Dominio, ItemDominio>((Dominio, ItemDominio) => Dominio.Codigo == ItemDominio.Codigo_Dominio)
                .Where<Dominio, ItemDominio>((Dominio, ItemDominio) => Dominio.Codigo > 1 && Dominio.Descricao.LIKE("TESTE_LIKE") 
                    && Dominio.Nome != null && Dominio.Nome == dominio.Nome)
                .OrderBy<Dominio>((Dominio => Dominio.Codigo))
                .OrderBy<ItemDominio>((ItemDominio) => ItemDominio.Nome)
                .GetQuery();

            Assert.AreEqual(query, "SELECT DISTINCT TOP(1) * FROM SignCi..CiDominio LEFT JOIN SignCi..CiItemDominio ON CiDominio.Codigo = CiItemDominio.Codigo_Dominio WHERE (((CiDominio.Codigo > 1 AND CiDominio.Descricao LIKE '%TESTE_LIKE%') AND CiDominio.Nome IS NOT NULL) AND CiDominio.Nome = 'Teste Nome') ORDER BY CiDominio.Codigo ASC, CiItemDominio.Nome ASC");
        }

        [TestMethod]
        public void Count()
        {
            var query = _dominioRepository.Select().Count()
                .Where((Dominio) => Dominio.Codigo > 1 && Dominio.Descricao.LIKE("TESTE_LIKE") && Dominio.Nome != null)
                .GetQuery();

            Assert.AreEqual(query, "SELECT COUNT(*) FROM SignCi..CiDominio WHERE ((CiDominio.Codigo > 1 AND CiDominio.Descricao LIKE '%TESTE_LIKE%') AND CiDominio.Nome IS NOT NULL)");
        }

        [TestMethod]
        public void Expression()
        {
            var query = _dominioRepository.Select()
        ...
        {
            var query = _dominioRepository.Select().UseAlias("d1").Top(1)
                .Join<Dominio, ItemDominio>((d1, i1) => (d1.Codigo == i1.Codigo_Dominio))
                .Where<Dominio, ItemDominio>((d1, i1) => d1.Codigo > 1 && i1.Descricao.LIKE(dominio.Descricao)
                    && (d1.Nome == dominio.Nome && d1.Descricao != null))
                .OrderBy<Dominio>((d1 => d1.Codigo))
                .OrderBy<ItemDominio>((i1) => i1.Nome)
                .GetQuery();
